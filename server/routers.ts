...
export const appRouter = router({
  ...
  answers: router({
    /** Get all answers for a student in a class */
    getMyAnswers: publicProcedure
      ...
      .query(async ({ input }) => {
        return getStudentAnswers(input.studentId);
      }),
    ...
    submit: publicProcedure
      ...
      .mutation(async ({ input }) => {
        // If teacher already revealed the answer, reject submission
        ...
        if (cls) {
          ...
          if (revealed.find(r => r.q === input.questionNumber)) {
            throw new Error("La risposta è già stata rivelata dal docente");
          }
        }

        // If student already answered this question, reject re-submission
        const existingAnswers = await getStudentAnswers(input.studentId);
        if (existingAnswers.find((a: any) => a.questionNumber === input.questionNumber)) {
