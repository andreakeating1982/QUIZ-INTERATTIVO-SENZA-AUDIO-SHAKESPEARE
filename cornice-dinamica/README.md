# CORNICE DINAMICA — QUIZ INTERATTIVO (embed per Blogger)

Questa cartella contiene la **cornice dinamica** dell'app QUIZ INTERATTIVO: un blocco
...
| `embed-shakespeare-quiz-lite.html` | **Versione leggera (~5 KB)**: font OpenDyslexic caricati dall'app via CORS. Codice piccolo e leggibile, ideale da incollare nel post. **Riutilizzabile**: per il quiz l'URL è già impostato; per un'altra app basta cambiare la riga `APP_URL` (o passare `?app=URL` nella pagina). Richiede che l'app sia online. |
...
| `embed-shakespeare-quiz.html` | **Versione autosufficiente (~110 KB)**: font incorporati in base64. Funziona anche se l'app è offline (i font restano) ed è robusta su qualsiasi piattaforma. |
...
2. Incolla l'intero contenuto di `embed-shakespeare-quiz-dedicata.html` (⭐ consigliata)
   oppure di `embed-shakespeare-quiz-lite.html` (minima) o di
   `embed-shakespeare-quiz.html` (autosufficiente).
...
| Caratteristica | `...-dedicata.html` | `...-lite.html` | `embed-shakespeare-quiz.html` |
|---|---|---|---|
| Dimensione | ~18 KB (v3 impermeabile + anti-loop) | ~5 KB | ~110 KB |
| Dedicata a Quiz Interattivo | ✅ sì (URL fisso) | riutilizzabile (`?app=`) | riutilizzabile |
...
1. **URL dell'app (file esistenti)**: nei file HTML cerca `https://shakespeare-quiz.easy-peasy.site`
...
`skills/shakespeare-quiz/SKILL.md` (sezioni 18–19).
...
- Il titolo "QUIZ INTERATTIVO" è in **OpenDyslexic Bold** (17px, letter-spacing 2px, colore plum),
